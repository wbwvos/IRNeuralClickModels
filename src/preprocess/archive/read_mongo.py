...
def upsert_query(query_id, serp, new_start=None):
...
def read_data(filename, new_start, new_end, number_to_be_processed):
    # bar = progressbar.ProgressBar(maxval=len(indices),
    #                               widgets=[progressbar.Bar('=', '[', ']'), ' ', progressbar.Percentage()])
    print "Starting reading datafile..."
    read_sessions(filename, new_start, new_end, number_to_be_processed)
...
if __name__ == "__main__":
    ...
    read_data("../../data/train", int(new_start), int(new_end), number_to_be_processed)
